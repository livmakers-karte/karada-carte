...
**本人の原因タイプに論理接続**する。区分は薬機法に従う（サプリ＝健康補助食品／シャンプー・エッセンス＝化粧品）。
※エスロッソ製品に医薬部外品（薬用）の承認・届出は無いため、「医薬部外品」表記は使わない（2026-07-05 BOSS確認）。

| 原因タイプ | 主因 | 接続する Esrosso 製品（すべて らくトク便＝定期） | 区分 |
|---|---|---|---|
| ① 内的要因型（ホルモン・加齢） | 体の内側 | **BOSTONサプリメント（＋アロマシャンプー付）** `gpn028` | 健康補助食品（食品） |
| ② 頭皮環境型（皮脂・洗浄） | 頭皮 | **GP.アロマシャンプー/トリートメント** `g-set-2-t` ＋ **スカルプエッセンス** `gnn063` | 化粧品 |
| ③ ハリ・コシ／栄養型（女性に多い） | 髪質 | **Verde アロマシャンプー/トリートメント（Ladies）** `v-set2-1-t` | 化粧品 |
| ④ 複合要因型 | 内＋外 | **BOSTONセット ＋ スカルプエッセンス**（内外併用＝客単価最大） | 併用 |
| 単品お試し（低ハードル） | — | **バオバブシャンプー3本**（非定期）`b-sha-3buy` | 化粧品 |

- 製品URLは `config.json`（`usuge.productUrls`／`usuge.offer.entryUrl`）に設定済み。**未確認の成分断定（「アミノ酸系」等）は使わない**。
- LTV設計：毛周期（数ヶ月〜年単位）の観点から「一定期間続けて経過をみる」ことを事実の範囲で説明し、
  **らくトク便（定期）** と **数ヶ月後の経過チェック診断（再診断・LINEなし運用時はフォーム＝メール案内）** を導線化。全額返金保証でリスクゼロ訴求。
...
- サプリ＝健康補助食品（食品）、シャンプー/エッセンス＝化粧品の**区分に応じた表現**のみ。医薬部外品の承認・届出は無いため「医薬部外品」表記は使わない。
...
特定の効果効能を保証するものではありません。エスロッソ製品は健康補助食品または化粧品の
区分に応じた範囲でご案内しています。気になる症状がある場合は医療機関にご相談ください。
